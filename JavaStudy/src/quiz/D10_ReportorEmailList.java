...
public class D10_ReportorEmailList {
	
	/*
	 	1. 어떤 도메인이 몇 개씩 있는지 세어서 출력해보기
	...
	public static void main(String[] args) {
		...
		while(matcher.find()) {			
			domainList.add(matcher.group(5).trim());
			fieldList.add(matcher.group(2).trim());
			mediaList.add(matcher.group(1).trim());
		}
		
		HashMap<String, Integer> domainCount = new HashMap<>();
		// 도메인 갯수 세기
		for (String domain : domainList) {
			Integer cnt = domainCount.putIfAbsent(domain, 1);
		...
		for (String key : domainCount.keySet()) {
			System.out.println("[" + key + " : " + domainCount.get(key) + "개]");
		}
		...
		for (String field : fieldList) {
			Integer cnt = fieldCount.putIfAbsent(field, 1);
		...
		for (String key : fieldCount.keySet()) {
			System.out.println("[" + key + " : " + fieldCount.get(key) + "개]");
		}
		
		System.out.println("==================================");
		
		HashMap<String, Integer> mediaCount = new HashMap<>();
		// 신문사 갯수 세기
		for (String media : mediaList) {
			Integer cnt = mediaCount.putIfAbsent(media, 1);
			
			if (cnt != null) {
				mediaCount.put(media, cnt + 1);
			}
		}
		
		// 출력
		for (String key : mediaCount.keySet()) {
			System.out.println("[" + key + " : " + mediaCount.get(key) + "개]");
		}
